...
export default function AdminReportsPage() {
  ...
  const filtered = (reports ?? []).filter((r: any) => {
    if (inScopeIds && !inScopeIds.has(String(r.storeId)) && !inScopeIds.has(String(r.targetId))) return false;
    const matchesSearch =
      !searchQuery ||
          ...
          <div className="space-y-3">
            {filtered.map((report: any) => {
              ...
              return (
                <Card
                  key={report.id}
                  className="border border-black/[0.06] rounded-xl bg-white overflow-hidden cursor-pointer hover:border-primary/20 transition-colors"
        ...
        >
          <DialogContent className="sm:max-w-[600px] rounded-3xl max-h-[90vh] overflow-y-auto">
            {selected && (
              <>
                <DialogHeader>
                  <DialogTitle className="font-semibold text-[#111] flex items-center gap-2">
                    {selected.category}
                    <Badge
                      variant="outline"
                      className={cn(
